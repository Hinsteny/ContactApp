...
import com.hinsteny.SunSnow.entity.Contact;
...
public class ContactDaoWebHandlers {
	...
	@WebActionHandler
	public Contact daoContactCreate(@WebParam("firstName") String firstName,
	...
	@WebActionHandler
	public Contact daoContactUpdate(@WebParam("contactId") Long contactId,
			@WebParam("firstName") String firstName,
			@WebParam("lastName") String lastName) {
	...
	@WebActionHandler
	public Contact daoContactSetGroup(@WebParam("contactId") Long contactId,
			@WebParam("groupIds") String groupIds) {
		if (groupIds != null) {
	...
	@WebActionHandler
	public Contact daoContactAddGroup(@WebParam("contactId") Long contactId,
			@WebParam("groupIds") String groupIds) {
		if (groupIds != null) {
			String stringarray[] = groupIds.split("\\|");
			ArrayList<Group> groups = new ArrayList<Group>();
			for (int i = 0; i < stringarray.length; i++) {
				Long id = Long.valueOf(stringarray[i]);
				contactDao.getContactStore().get(contactId).getGroups()
						.add(groupDao.get(id));
				// groups.add(groupDao.get(id));
			}
